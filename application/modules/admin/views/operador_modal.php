				...
				<div class="form-group text-left">
					...
					<select name="depto" id="depto" class="form-control" >
						...
						<?php for ($i = 0; $i < count($departamentos); $i++) { ?>
							<option value="<?php echo $departamentos[$i]["codigo_departamento"]; ?>" <?php if($information[0]["codigo_departamento"] == $departamentos[$i]["codigo_departamento"]) { echo "selected"; }  ?>><?php echo $departamentos[$i]["nombre_departamento"]; ?></option>	
						<?php } ?>
					</select>
				</div>
			</div>
				...
				<div class="form-group text-left">
					...
					<select name="usuario" id="usuario" class="form-control" required>
						...
						<?php for ($i = 0; $i < count($usuarios); $i++) { ?>
							<option value="<?php echo $usuarios[$i]["id_usuario"]; ?>" <?php if($information[0]["fk_id_operador_mcpio"] == $usuarios[$i]["id_usuario"]) { echo "selected"; }  ?>><?php echo  "C.C. " . $usuarios[$i]["numero_documento"] . " - " . $usuarios[$i]["nombres_usuario"] . " " . $usuarios[$i]["apellidos_usuario"]; ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
